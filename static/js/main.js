...
var viewModel = function(data, client_id){
    ...
    if(data.length >0){ 
        $.each(data,function(inx,v){
            ...
            obj.Description(v.Description);obj.priority(v.priority);
            obj.target_date(new Date(v.target_date)); obj.area_name(v.area_name);
    ...
    self.addLine = function(){ console.log("addd");
    ...
    self.save = function(){
        ...
        $.ajax({
            ...
            data:{'all_features':ko.toJSON(self.lines())},
            success:function(){
...
$('tbody').on('click','.delelem',function(){
    tobe = parseInt(this.value); //n
    if(isNaN(tobe))
        return;
    curr = parseInt($(this).prev().val()); //p
